...
interface NewProjectWizardProps {
  onBack: () => void;
  onPublish: (input: { name: string; description: string; instructions: string; fields: FieldDefinition[]; emails: string[] }) => void | Promise<void>;
}

const wizardSteps = ["Identity", "Schema", "Contributors", "Publish"];
const fieldTypes: FieldDefinition["type"][] = ["short_text", "long_text", "number", "single_choice", "multiple_choice", "tri_state", "date", "datetime", "location", "photo", "audio", "repeatable_group"];
...
export function NewProjectWizard({ onBack, onPublish }: NewProjectWizardProps) {
  ...
  const [emails, setEmails] = useState("marco@example.com\nsara@example.com");
  const [fields, setFields] = useState<FieldDefinition[]>(projectFields);
  ...
  const addField = () => {
    ...
    setFields((current) => [...current, { id, key: `field_${current.length + 1}`, label: "New field", type: "short_text" }]);
  };
