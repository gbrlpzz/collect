...
export function ContributorHome({
  ...
  return (
    <main className="page page-contributor">
      ...
      <section className="project-list" aria-label="Assigned projects">
        {projects.length ? (
              ...
              <div className="project-row-wrap" key={project.id}>
                ...
                >
                  <span className="project-row-mark" aria-hidden="true">
                    {project.organizationMark}
                  </span>
                  <span className="project-row-copy">
                    <strong>{project.name}</strong>
                    <span>
                      {project.organization} · {project.description}
                    </span>
                  </span>
                ...
                {!ready && waitingCount === 0 && (
                  <button
                    className="download-offline-button"
                    onClick={() => onMakeAvailableOffline(project)}
                  >
          ...
          <div className="empty-list-state">
            ...
            <span>
              Your administrator will send an invitation when fieldwork is
              ready.
            </span>
          </div>
        )}
      ...
      {projects.length > 0 && (
        <p className="page-note">Saved on this device before it is sent.</p>
      )}
    </main>
  );
}
