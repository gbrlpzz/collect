...
export async function uploadRemoteMedia({ observation, project, asset }: { observation: Observation; project: Project; asset: MediaAsset }): Promise<void> {
  ...
  const existing = await invoke("sync-submission", { action: "confirm_media", submission_id: observation.id, media_id: asset.id, object_path: objectName }, z.object({ confirmed: z.boolean() }));
  if (existing.confirmed) return;
  const { data: sessionData } = await client.auth.getSession();
  const accessToken = sessionData.session?.access_token;
  if (!accessToken) throw new Error("Authentication is required before media upload");
  const projectUrl = import.meta.env.VITE_SUPABASE_URL as string;
  const publishableKey = (import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY ?? import.meta.env.VITE_SUPABASE_ANON_KEY) as string;
  ...
  await new Promise<void>((resolve, reject) => {
    const upload = new Upload(asset.blob!, {
      ...
      fingerprint: () => Promise.resolve(`collect:${objectName}:${asset.byteSize}:${asset.sha256 ?? ""}`),
  ...
  const confirmation = await invoke("sync-submission", { action: "confirm_media", submission_id: observation.id, media_id: asset.id, object_path: objectName }, z.object({ confirmed: z.boolean() }));
  if (!confirmation.confirmed) throw new Error(`Media ${asset.id} was uploaded but not acknowledged by the server`);
}
...
export async function finalizeRemoteSubmission({ observation }: { observation: Observation }): Promise<RemoteReceipt> {
  const receipt = await invoke("sync-submission", { action: "finalize_submission", submission_id: observation.id }, receiptSchema);
  if (receipt.status !== "COMPLETE") throw new Error("Server did not return a complete receipt");
  return receipt;
}
...
export async function probeRemoteHealth(): Promise<boolean> {
  ...
  try {
    const response = await fetch(`${projectUrl.replace(/\/$/, "")}/functions/v1/health`, {
      method: "HEAD",
...
export async function reportDeviceStatus(payload: Record<string, unknown>): Promise<void> {
