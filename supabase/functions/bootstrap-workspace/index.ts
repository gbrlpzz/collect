...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (bootstrapEmail && user.email?.trim().toLowerCase() !== bootstrapEmail) {
      return json({ error: "This account is not configured as the first administrator" }, { status: 403 });
    }

    const body = await request.json() as Record<string, unknown>;
    const organizationName = String(body.organization_name ?? "").trim();
    if (!organizationName || organizationName.length > 160) {
      return json({ error: "A workspace name between 1 and 160 characters is required" }, { status: 400 });
    }
    ...
    if (error) {
      if (/workspace already exists/i.test(error.message)) {
        return json({ error: "This deployment already has a workspace. Ask an existing administrator to grant this account admin access." }, { status: 409 });
      }
      return json({ error: "The first workspace could not be created" }, { status: 500 });
    }

    const row = Array.isArray(data) ? data[0] : data;
    if (!row?.organization_id) return json({ error: "The first workspace could not be created" }, { status: 500 });
    return json({ organization_id: row.organization_id, organization_name: row.organization_name });
  } catch (error) {
