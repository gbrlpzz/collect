...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!projectId || !email || !email.includes("@")) return json({ error: "Project and contributor email are required" }, { status: 400 });
    ...
    const { data: existingInvite } = await service.from("project_invites").select("id").eq("project_id", projectId).eq("status", "pending").ilike("email", email).maybeSingle();
    ...
    const { error: insertError } = await service.from("project_invites").insert({ project_id: projectId, email, invited_by: user.id, invited_user_id: invitedUserId });
    if (insertError) return json({ error: "The invitation record could not be stored" }, { status: 500 });
    if (invitedUserId) await service.from("project_members").upsert({ project_id: projectId, user_id: invitedUserId, role: "contributor" }, { onConflict: "project_id,user_id" });
    await service.from("audit_events").insert({ organization_id: access.project.organization_id, project_id: projectId, actor_id: user.id, action: "contributor_invited", metadata: { email } });
